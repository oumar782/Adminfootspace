...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`crenau-toast crenau-toast-${type}`}>
      <span className="crenau-toast-icon">
        {type === 'success' ? '✓' : '✗'}
      </span>
      <span className="crenau-toast-message">{message}</span>
      <button className="crenau-toast-close" onClick={onClose}>×</button>
    </div>
  );
};
...
const ViewModal = ({ creneau, onClose }) => {
  ...
  return (
    <div className="crenau-modal-overlay" onClick={onClose}>
      <div className="crenau-modal-content" onClick={e => e.stopPropagation()}>
        <div className="crenau-modal-header">
          <h2>Détails du créneau</h2>
          <button className="crenau-modal-close" onClick={onClose}>×</button>
        </div>
        <div className="crenau-modal-body">
          <div className="crenau-detail-grid">
            <div className="crenau-detail-item">
              <label>ID:</label>
            ...
            <div className="crenau-detail-item">
              <label>Statut:</label>
              <span className={`crenau-status-badge crenau-status-${creneau.statut}`}>
                {creneau.statut}
              </span>
            </div>
            <div className="crenau-detail-item">
        ...
        <div className="crenau-modal-footer">
          <button className="crenau-btn crenau-btn-secondary" onClick={onClose}>
            Fermer
          </button>
        </div>
      </div>
...
const CreneauModal = ({ creneau, onClose, onSubmit, isEditing }) => {
  ...
  return (
    <div className="crenau-modal-overlay" onClick={onClose}>
      <div className="crenau-modal-content crenau-compact" onClick={e => e.stopPropagation()}>
        <div className="crenau-modal-header">
          <h2>{isEditing ? 'Modifier un créneau' : 'Ajouter un créneau'}</h2>
          <button className="crenau-modal-close" onClick={onClose}>×</button>
        </div>
        <div className="crenau-modal-body">
          <form onSubmit={handleSubmit} className="crenau-form">
            <div className="crenau-form-row">
              <div className="crenau-form-group">
            ...
            <div className="crenau-form-row">
              <div className="crenau-form-group crenau-full-width">
            ...
            <div className="crenau-form-actions">
              <button type="submit" className="crenau-btn crenau-btn-primary">
                {isEditing ? 'Modifier' : 'Ajouter'}
              </button>
              <button type="button" className="crenau-btn crenau-btn-secondary" onClick={onClose}>
                Annuler
              </button>
            </div>
          </form>
...
const Crenau = () => {
  ...
  return (
    <div className="crenau-app">
      ...
      <main className="crenau-main">
        {/* Cartes de statistiques */}
        {stats && (
          <section className="crenau-stats-section">
            <div className="crenau-stats-grid">
              <div className="crenau-stat-card">
                <div className="crenau-stat-value">{stats.total_creneaux}</div>
                <div className="crenau-stat-label">Total créneaux</div>
              </div>
              <div className="crenau-stat-card">
                <div className="crenau-stat-value">{stats.disponibles}</div>
                <div className="crenau-stat-label">Disponibles</div>
              </div>
              <div className="crenau-stat-card">
                <div className="crenau-stat-value">{stats.reserves}</div>
                <div className="crenau-stat-label">Réservés</div>
              </div>
          
              <div className="crenau-stat-card">
                <div className="crenau-stat-value">{stats.terrains_actifs}</div>
                <div className="crenau-stat-label">Terrains actifs</div>
              </div>
              <div className="crenau-stat-card">
                <div className="crenau-stat-value">{parseFloat(stats.tarif_moyen).toFixed(2)}DH</div>
                <div className="crenau-stat-label">Tarif moyen</div>
              </div>
            </div>
          </section>
        )}
        ...
        {/* Actions principales */}
        <section className="crenau-actions-section">
          <div className="crenau-section-header">
            <h2>Gestion des créneaux</h2>
            <button className="crenau-btn crenau-btn-primary" onClick={openAddModal}>
              Nouveau créneau
            </button>
          </div>
        </section>
        ...
        {/* Filtres */}
        <section className="crenau-filters-section">
          <h2>Filtres</h2>
          <div className="crenau-filters">
            <div className="crenau-filter-group">
            ...
            <div className="crenau-filter-actions">
              <button 
                className="crenau-btn crenau-btn-primary" 
                onClick={fetchFilteredCreneaux}
              >
                Appliquer
              </button>
              <button 
                className="crenau-btn crenau-btn-secondary" 
                onClick={() => {
        ...
        <section className="crenau-table-section">
          <div className="crenau-section-header">
            <h2>Liste des créneaux ({creneaux.length})</h2>
            <button className="crenau-btn crenau-btn-outline" onClick={fetchCreneaux}>
              Actualiser
            </button>
          </div>
          
          {loading ? (
            <div className="crenau-loading">
              <div className="crenau-spinner"></div>
              <p>Chargement des créneaux...</p>
            </div>
          ) : (
            <div className="crenau-table-container">
              <table className="crenau-table">
                <thead>
                  <tr>
                    <th>ID</th>
                    <th>Date</th>
                ...
                <tbody>
                  {creneaux.length === 0 ? (
                    <tr>
                      <td colSpan="11" className="crenau-no-data">
                        <div className="crenau-no-data-content">
                    ...
                    creneaux.map((creneau, index) => (
                      <tr key={creneau.idcreneaux} className="crenau-fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
                        ...
                        <td className="crenau-actions">
                          <button 
                            className="crenau-btn-icon crenau-view" 
                          ...
                          <button 
                            className="crenau-btn-icon crenau-edit" 
                          ...
                          <button 
                            className="crenau-btn-icon crenau-delete" 

...
const GestionAbonnes = () => {
  ...
  const getStatusBadge = (statut) => {
    switch(statut) {
      case 'actif': 
        return { icon: <CheckCircle size={14} />, text: 'Actif', class: 'ga-badge-active' };
      case 'inactif': 
        return { icon: <XCircle size={14} />, text: 'Inactif', class: 'ga-badge-inactive' };
      case 'en attente': 
        return { icon: <Clock size={14} />, text: 'En attente', class: 'ga-badge-pending' };
      case 'expire':
        return { icon: <AlertCircle size={14} />, text: 'Expiré', class: 'ga-badge-expired' };
      default: 
    ...
    <div className="ga-root">
      ...
      <div className="ga-header">
        <div className="ga-header-left">
          ...
          <div className="ga-header-title">
            ...
            <p>
              <span className="ga-count">{filteredData.length}</span> abonné{filteredData.length !== 1 ? 's' : ''}
              <span className="ga-separator">•</span>
              <span className="ga-active-count">{data.filter(d => d.statut === 'actif').length}</span> actifs
            </p>
          </div>
        </div>
        <div className="ga-header-actions">
          <button className="ga-btn ga-btn-outline" onClick={fetchData}>
            <RefreshCw size={16} />
            Actualiser
          </button>
          <button className="ga-btn ga-btn-primary" onClick={() => openModal('add')}>
            <UserPlus size={16} />
            Nouvel abonné
          </button>
        </div>
      ...
      <div className="ga-filters">
        <div className="ga-search-box">
          <Search size={18} className="ga-search-icon" />
          <input
            type="text"
          ...
          />
          {searchTerm && (
            <button className="ga-search-clear" onClick={() => setSearchTerm('')}>
              <X size={14} />
            </button>
          )}
        </div>
        ...
        <div className="ga-filter-group">
          <Filter size={18} className="ga-filter-icon" />
          <select 
        ...
        <div className="ga-view-toggle">
          <button
            className={`ga-toggle-btn ${viewMode === 'grid' ? 'ga-active' : ''}`}
            onClick={() => setViewMode('grid')}
          >
            <LayoutGrid size={18} />
          </button>
          <button
            className={`ga-toggle-btn ${viewMode === 'table' ? 'ga-active' : ''}`}
      ...
      <div className="ga-content">
        {isLoading ? (
          ...
          <div className="ga-empty">
            ...
            <button className="ga-btn ga-btn-primary" onClick={() => { setSearchTerm(''); setStatusFilter('all'); }}>
              Réinitialiser
            </button>
          </div>
        ) : viewMode === 'grid' ? (
          <div className="ga-grid">
            {paginatedData.map((item, index) => {
              ...
              return (
                <div key={item.idclient} className="ga-card" style={{ animationDelay: `${index * 0.05}s` }}>
                  {/* Badges */}
                  <div className="ga-card-badges">
                    {isNew && (
                      <span className="ga-badge ga-badge-new">
                        <Sparkles size={10} /> Nouveau
                      </span>
                    )}
                    {isExpiring && (
                      <span className="ga-badge ga-badge-expiring">
                  ...
                  <div className="ga-card-header">
                    <div className="ga-card-avatar">
                      {getInitials(item.nom, item.prenom)}
                    </div>
                    <div className="ga-card-id">#{item.idclient}</div>
                  </div>
                  
                  {/* Corps de la carte */}
                  <div className="ga-card-body">
                    <h3 className="ga-card-name">{item.nom} {item.prenom}</h3>
                    
                    <div className="ga-card-status">
                      <span className={`ga-status-badge ${status.class}`}>
                    ...
                    <div className="ga-card-info">
                      ...
                      {item.prix_total && (
                        <div className="ga-info-row ga-price-row">
                          <DollarSign size={12} />
                          <span className="ga-price">{item.prix_total} DH</span>
                        </div>
                      )}
                  ...
                  <div className="ga-card-footer">
                    <button className="ga-action ga-action-view" onClick={() => openModal('view', item)}>
                      <Eye size={14} /> Détails
                    </button>
                    <button className="ga-action ga-action-edit" onClick={() => openModal('edit', item)}>
                      <Pencil size={14} /> Modifier
                    </button>
                    <button className="ga-action ga-action-delete" onClick={() => openModal('delete', item)}>
                      <Trash2 size={14} /> Supprimer
                    </button>
                  </div>

                  {/* Effet de brillance */}
                  <div className="ga-card-shine"></div>
                </div>
              );
            })}
          </div>
              ...
              <thead>
                <tr>
                  <th onClick={() => handleSort('idclient')}>
                    <Hash size={12} />
                    ID {sortConfig.field === 'idclient' && <ArrowUpDown size={12} className={`ga-sort-${sortConfig.direction}`} />}
                  </th>
                  <th onClick={() => handleSort('nom')}>
                    <User size={12} />
                    Nom {sortConfig.field === 'nom' && <ArrowUpDown size={12} className={`ga-sort-${sortConfig.direction}`} />}
                  </th>
                  <th onClick={() => handleSort('prenom')}>
                    Prénom {sortConfig.field === 'prenom' && <ArrowUpDown size={12} className={`ga-sort-${sortConfig.direction}`} />}
                  </th>
                  <th>Contact</th>
                  <th onClick={() => handleSort('type_abonnement')}>
                    <Tag size={12} />
                    Abonnement {sortConfig.field === 'type_abonnement' && <ArrowUpDown size={12} className={`ga-sort-${sortConfig.direction}`} />}
                  </th>
                  <th onClick={() => handleSort('prix_total')}>
                    <DollarSign size={12} />
                    Prix {sortConfig.field === 'prix_total' && <ArrowUpDown size={12} className={`ga-sort-${sortConfig.direction}`} />}
                  </th>
                  <th>
                    <Calendar size={12} />
                    Période
                  ...
                  <th onClick={() => handleSort('heure_reservation')}>
                    <Clock size={12} />
                    Horaire {sortConfig.field === 'heure_reservation' && <ArrowUpDown size={12} className={`ga-sort-${sortConfig.direction}`} />}
                  </th>
                  <th>Statut</th>
                  <th>Actions</th>
                </tr>
                ...
                {paginatedData.map(item => {
                  ...
                  return (
                    <tr key={item.idclient}>
                      <td className="ga-table-id">#{item.idclient}</td>
                      <td>{item.nom}</td>
                      <td>{item.prenom}</td>
                      <td>
                        <div className="ga-table-contact">
                      ...
                      <td>
                        {item.type_abonnement && (
                          <span className="ga-table-badge">
                            {getTypeIcon(item.type_abonnement)} {item.type_abonnement}
                          </span>
                        )}
                      </td>
                      <td className="ga-table-price">{item.prix_total ? `${item.prix_total} DH` : '-'}</td>
                      <td className="ga-table-period">
                      ...
                      <td>
                        {heures && (
                          <span className="ga-table-heures">
                      ...
                      <td className="ga-table-actions">
                        <button className="ga-icon-btn ga-view" onClick={() => openModal('view', item)}>
                          <Eye size={14} />
                        </button>
                        <button className="ga-icon-btn ga-edit" onClick={() => openModal('edit', item)}>
                          <Pencil size={14} />
                        </button>
                        <button className="ga-icon-btn ga-delete" onClick={() => openModal('delete', item)}>
                          <Trash2 size={14} />
                        </button>
                      </td>
                    </tr>
      ...
      {filteredData.length > 0 && (
        <div className="ga-pagination">
          ...
          <div className="ga-pagination-controls">
            ...
            <span className="ga-page-numbers">{currentPage} / {totalPages}</span>
            <button 
              className="ga-pagination-btn" 
              onClick={() => setCurrentPage(p => p + 1)} 
      ...
      {(modalState.add || modalState.edit) && (
        <div className="ga-modal-overlay" onClick={() => closeModal(modalState.add ? 'add' : 'edit')}>
          <div className="ga-modal" onClick={e => e.stopPropagation()}>
            <div className="ga-modal-header">
              ...
              <button className="ga-modal-close" onClick={() => closeModal(modalState.add ? 'add' : 'edit')}>
                <X size={18} />
              </button>
            </div>
              ...
              <form onSubmit={(e) => { e.preventDefault(); modalState.add ? handleAdd() : handleEdit(); }}>
                <div className="ga-form-row">
                  <div className="ga-form-group">
                    <label>Nom <span className="ga-required">*</span></label>
                    <input
                      type="text"
                      value={formData.nom}
                  ...
                  <div className="ga-form-group">
                    <label>Prénom <span className="ga-required">*</span></label>
                    <input
                      type="text"
                ...
                <div className="ga-form-row">
                  <div className="ga-form-group">
                    <label>Email <span className="ga-required">*</span></label>
                    <input
                      type="email"
                  ...
                  <div className="ga-form-group">
                    <label>Téléphone <span className="ga-required">*</span></label>
                    <input
                      type="tel"
            ...
            <div className="ga-modal-footer">
              <button className="ga-btn ga-btn-secondary" onClick={() => closeModal(modalState.add ? 'add' : 'edit')}>
                Annuler
              </button>
              <button className="ga-btn ga-btn-primary" onClick={modalState.add ? handleAdd : handleEdit}>
      ...
      {modalState.view && selectedItem && (
        <div className="ga-modal-overlay" onClick={() => closeModal('view')}>
          <div className="ga-modal ga-view-modal" onClick={e => e.stopPropagation()}>
            <div className="ga-modal-header">
              <h3><Eye size={20} /> Détails de l'abonné</h3>
              <button className="ga-modal-close" onClick={() => closeModal('view')}>
                <X size={18} />
              </button>
            </div>
            
            <div className="ga-modal-body">
              <div className="ga-view-profile">
                <div className="ga-view-avatar">
                  {getInitials(selectedItem.nom, selectedItem.prenom)}
                </div>
                <div className="ga-view-info">
                  <h2>{selectedItem.nom} {selectedItem.prenom}</h2>
                  {(() => {
                    const status = getStatusBadge(selectedItem.statut);
                    return <span className={`ga-status-badge ${status.class}`}>{status.icon} {status.text}</span>;
                  })()}
                </div>
              </div>

              <div className="ga-view-details">
                <div className="ga-view-section">
                  <h4>📋 Informations générales</h4>
                  <div className="ga-view-row">
                    <span className="ga-view-label">ID Client</span>
                    <span className="ga-view-value">#{selectedItem.idclient}</span>
                  </div>
                  <div className="ga-view-row">
                    <span className="ga-view-label">Email</span>
                    <span className="ga-view-value">{selectedItem.email}</span>
                  </div>
                  <div className="ga-view-row">
                    <span className="ga-view-label">Téléphone</span>
                    <span className="ga-view-value">{selectedItem.telephone}</span>
                  </div>
                </div>
                ...
                <div className="ga-view-section">
                  <h4>📦 Abonnement</h4>
                  <div className="ga-view-row">
                    <span className="ga-view-label">Type</span>
                    <span className="ga-view-value">{selectedItem.type_abonnement || '-'}</span>
                  </div>
                  <div className="ga-view-row">
                    <span className="ga-view-label">Prix</span>
                    <span className="ga-view-value ga-price">{selectedItem.prix_total ? `${selectedItem.prix_total} DH` : '-'}</span>
                  </div>
                  <div className="ga-view-row">
                    <span className="ga-view-label">Date début</span>
                    <span className="ga-view-value">{formatDate(selectedItem.date_debut)}</span>
                  </div>
                  <div className="ga-view-row">
                    <span className="ga-view-label">Date fin</span>
                    <span className="ga-view-value">{formatDate(selectedItem.date_fin)}</span>
                  </div>
                </div>
                ...
                <div className="ga-view-section">
                  <h4>💰 Paiement</h4>
                  <div className="ga-view-row">
                    <span className="ga-view-label">Mode</span>
                    <span className="ga-view-value">{selectedItem.mode_paiement || '-'}</span>
                  </div>
                </div>
                ...
                {(selectedItem.heure_reservation || selectedItem.heure_fin) && (
                  <div className="ga-view-section">
                    ...
                    <div className="ga-view-heures">
                      <div className="ga-heure-item">
                        <span className="ga-heure-label">Début</span>
                        <span className="ga-heure-value">{formatHeure(selectedItem.heure_reservation)}</span>
                      </div>
                      {selectedItem.heure_reservation && selectedItem.heure_fin && (
                        <div className="ga-heure-separator">→</div>
                      )}
                      <div className="ga-heure-item">
                        <span className="ga-heure-label">Fin</span>
                        <span className="ga-heure-value">{formatHeure(selectedItem.heure_fin)}</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="ga-modal-footer">
              <button className="ga-btn ga-btn-secondary" onClick={() => closeModal('view')}>
                Fermer
              </button>
              <button className="ga-btn ga-btn-primary" onClick={handlePrint}>
                <Printer size={16} /> Imprimer
              </button>
              <button className="ga-btn ga-btn-primary" onClick={() => {
                closeModal('view');
      ...
      {modalState.delete && selectedItem && (
        <div className="ga-modal-overlay" onClick={() => closeModal('delete')}>
          <div className="ga-modal ga-delete-modal" onClick={e => e.stopPropagation()}>
            <div className="ga-modal-header">
              <h3><Trash2 size={20} /> Confirmation</h3>
              <button className="ga-modal-close" onClick={() => closeModal('delete')}>
                <X size={18} />
              </button>
            </div>
            
            <div className="ga-modal-body">
              <div className="ga-delete-content">
                <AlertCircle size={64} />
                <p>
                  Êtes-vous sûr de vouloir supprimer <strong>{selectedItem.nom} {selectedItem.prenom}</strong> ?
                </p>
                <p className="ga-warning">Cette action est irréversible</p>
              </div>
            </div>

            <div className="ga-modal-footer">
              <button className="ga-btn ga-btn-secondary" onClick={() => closeModal('delete')}>
                Annuler
              </button>
              <button className="ga-btn ga-btn-danger" onClick={handleDelete}>
      ...
      {/* Notification */}
      {notification && (
        <div className={`ga-notification ga-notification-${notification.type}`}>

...
const OccupationChart = () => {
  ...
  const renderChartSkeleton = () => (
    <div className="occ-chart-skeleton">
      <div className="occ-skeleton-header"></div>
      <div className="occ-skeleton-chart">
        {Array.from({ length: period === 'day' ? 8 : period === 'week' ? 7 : 4 }).map((_, i) => (
          <div 
            key={i} 
            className="occ-skeleton-bar"
            style={{ 
      ...
      <div className="occ-skeleton-footer">
        <div className="occ-skeleton-stats"></div>
        <div className="occ-skeleton-stats"></div>
      </div>
    </div>
  );
  ...
  if (error) {
    return (
      <div className="occ-chart-card">
        <div className="occ-error-message">
          <AlertCircle size={48} className="occ-error-icon" />
          <p>Erreur de chargement: {error}</p>
          <button onClick={fetchReservations} className="occ-retry-btn">
            <RefreshCw size={16} />
            Réessayer
          </button>
        </div>
  ...
  return (
    <div className="occ-chart-card">
      <div className="occ-chart-header">
        <div className="occ-header-content">
          <div className="occ-title-section">
            <div className="occ-header-icon-container">
              <BarChart3 size={28} className="occ-header-icon" />
            </div>
            <div className="occ-title-text">
              <h2>Tableau de Bord d'Occupation</h2>
              <p className="occ-chart-subtitle">
                Surveillance en temps réel des réservations et de l'occupation des terrains
              </p>
            </div>
          </div>
        </div>
        <div className="occ-period-selector">
          <button 
            className={`occ-period-btn ${period === 'day' ? 'occ-active' : ''}`}
          ...
          <button 
            className={`occ-period-btn ${period === 'week' ? 'occ-active' : ''}`}
          ...
          <button 
            className={`occ-period-btn ${period === 'month' ? 'occ-active' : ''}`}
      ...
      <div className="occ-chart-content">
        {isLoading ? (
          renderChartSkeleton()
        ) : (
          <>
            <div className="occ-chart-container">
              <div className="occ-chart-bars">
                {data.map((item, index) => (
                  <div key={index} className="occ-bar-container">
                    <div 
                      className="occ-bar-background" 
                      style={{ height: '100%' }}
                    ></div>
                    <div 
                    ...
                    >
                      <div className="occ-occupation-label">
                        <span className="occ-occupation-percent">{item.occupation}%</span>
                        <span className="occ-occupation-details">
                          {item.count}/{item.max}
                        </span>
                      </div>
                    </div>
                    <div className="occ-bar-label">{item.time}</div>
                  </div>
                ))}
              </div>
              
              {hoveredData && (
                <div className="occ-chart-tooltip">
                  <div className="occ-tooltip-content">
                    <div className="occ-tooltip-header">
                      <span className="occ-tooltip-title">{hoveredData.time}</span>
                      <div className="occ-tooltip-indicator">
                        <div className="occ-indicator-dot"></div>
                        {hoveredData.occupation}% d'occupation
                      </div>
                    </div>
                    <div className="occ-tooltip-details">
                      <div className="occ-detail-item">
                        <Users size={14} />
            ...
            <div className="occ-chart-stats">
              <div className="occ-stat-item">
                <div className="occ-stat-icon occ-average">
                  <TrendingUp size={20} />
                </div>
                <div className="occ-stat-content">
                  <div className="occ-stat-labels">Occupation moyenne</div>
                  <div className="occ-stat-values">{averageOccupation}%</div>
                  <div className="occ-stat-bar">
                    <div 
                      className="occ-stat-bar-fill" 
              ...
              <div className="occ-stat-item">
                <div className="occ-stat-icon occ-peak">
                  <Flame size={20} />
                </div>
                <div className="occ-stat-content">
                  <div className="occ-stat-labels">Pic d'occupation</div>
                  <div className="occ-stat-values">{peakOccupation}%</div>
                  <div className="occ-stat-bar">
                    <div 
                      className="occ-stat-bar-fill occ-peak" 
              ...
              <div className="occ-stat-item">
                <div className="occ-stat-icon occ-period">
                  <Zap size={20} />
                </div>
                <div className="occ-stat-content">
                  <div className="occ-stat-labels">Période analysée</div>
                  <div className="occ-stat-values">
                    {period === 'day' ? 'Aujourd\'hui' : 
                     period === 'week' ? 'Cette semaine' : 
                     'Ce mois-ci'}
                  </div>
                  <div className="occ-stat-period">
            ...
            <div className="occ-data-info">
              <div className="occ-data-summary">
                <span className="occ-data-badge">
                  <Target size={14} />
              ...
              <button onClick={fetchReservations} className="occ-refresh-btn">
                <RefreshCw size={16} />
                Actualiser
              </button>
            </div>
